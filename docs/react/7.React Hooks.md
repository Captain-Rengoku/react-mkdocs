...
- `useId` – Generates unique IDs for accessibility & form inputs.
- `useDeferredValue` – Defers state updates for smoother performance.
- `useTransition` – Handles UI transitions efficiently and Optimize state updates.
- `useSyncExternalStore` – Connect to external state stores
